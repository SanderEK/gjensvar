...
export default function SetupPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 8) {
      setError(t("setupPassword.errorTooShort"));
      return;
    }

    if (password !== confirmPassword) {
      setError(t("setupPassword.errorMismatch"));
      return;
    }
    ...
    if (updateError) {
      setError(t("setupPassword.errorGeneric"));
      setLoading(false);
      return;
    }
          ...
          <div className="rounded-2xl border border-zinc-800/80 bg-[#131313] p-7 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              >
                {loading ? (
                  <>
                    <Spinner /> {t("setupPassword.submitting")}
                  </>
                ) : (
                  <>
